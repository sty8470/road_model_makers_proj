...
def create_node_set_and_link_set(map_info, origin):
    ...
    node_set = NodeSet()
    for i, item in enumerate(map_info[node_file_name]['features']):
    ...
    link_set = LineSet()
    for i, item in enumerate(map_info[link_file_name]['features']):
        obj_prop = item['properties'] # item와 매우 유사
...
def set_lane_change_info_into_link_set(map_info, link_set):
    ...
    '''
    link_file_name = next((item for item in map_info.keys() if 'LINK' in item), False)
    a2_link = map_info[link_file_name]['features']
    lane_ch_link_set = LineSet()
    for i, item in enumerate(a2_link):
        obj_prop = item['properties'] # item와 매우 유사
...
def find_obj_using_property(geojson_layer, prop_name, prop_value):
    ...
    """
    for i, item in enumerate(geojson_layer):
        obj_prop = item['properties'] # item와 매우 유사
...
def get_traffic_sign_asset_path_and_name(prop_type, prop_subtype, link=None):
    ...
    if prop_type == 1:
        file_path = '01_Caution_Beam'
        file_name = '01_Caution_{}_Beam.prefab'.format(prop_subtype)


    elif prop_type == 2:
        file_path = '02_Restriction_Beam'
...
def get_surface_marking_asset_path_and_name(prop_type, inspect_model_exists=True):
    ...
    file_path = 'KR_SurfaceMarking_New'
...
def create_cw_set(sf, origin):

    cw_set = CrossWalkSet()

    for i in sf:
        cw_id = i['properties']['id']
        points = np.array(i['geometry']['coordinates'][0])
        cw_type = i['properties']['Type']
        sub_type = i['properties']['Kind']
        points -= origin
