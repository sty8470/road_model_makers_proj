...
class FindJunctionWindow(QDialog):
    def __init__(self):
        super().__init__()
        self.initUI()
    ...
    def initUI(self):
        ...
        self.setModal = True
        self.setWindowTitle('Find Junction')

        self.width = 700
        self.height = 500

        self.cbJunctionID = QCheckBox('', self)
        self.cbJunctionID.toggle()

        self.txtJunctionID = QLineEdit('', self)
        ...
        self.lbJunctionID = QLabel('Junction ID : ')      
        self.lbJunctionID.setBuddy(self.txtJunctionID)
        
        gridLayout = QGridLayout()
        gridLayout.addWidget(self.cbJunctionID, 0, 0)
        gridLayout.addWidget(self.lbJunctionID, 0, 1)
        gridLayout.addWidget(self.txtJunctionID, 0, 2)
    ...
    def getParameters(self):
        ...
        if self.cbJunctionID.isChecked():
            junction_id = self.txtJunctionID.text()

        search_conditions = {
            'junction_id': {
              'checked': self.cbJunctionID.isChecked(),
              'val': junction_id
            }
        }
