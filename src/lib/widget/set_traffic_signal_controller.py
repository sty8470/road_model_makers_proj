...
from lib.openscenario.class_defs.global_action import Phase, TrafficSignalState
...
class DrawIntTLCtrl(QOpenGLWidget):
    ...
    def highlight_select_item(self, _id, color=[220/255, 240/255, 0]):
        tl_list = self.draw_item
        if _id is not None:
...
class SettingIntersectionSchedule(QDialog):
    def __init__(self, mgeo, id):
        ...
        self._width = 150
        self._height = 260    
        self.int_item = mgeo.intersection_controller_set.intersection_controllers[id]
        try:
    ...
    def set_tl_table_widget(self):
        ...
        self.tl_table.verticalHeader().sectionDoubleClicked.connect(self.change_vertical_header)
        ...
        append_state = QPushButton("Add Phase")
        grid.addWidget(append_state, 0, 0)
        delete_state = QPushButton("Delete Phase")
        grid.addWidget(delete_state, 0, 1)

        append_state.clicked.connect(self.__append_phase)
        delete_state.clicked.connect(self.__delete_phase)
        main_widget.setLayout(vbox)
        return main_widget

    def change_vertical_header(self, index):
        old_header = self.tl_table.verticalHeaderItem(index).text()
        new_header, ok = QInputDialog.getText(self, 'Change Phase Label %d' % index,
                                                    'Phase:',
                                                    QLineEdit.Normal,
                                                    old_header)
        if ok:
            self.tl_table.verticalHeaderItem(index).setText(new_header)
    ...
    def traffic_light_update_table_widget(self):
        ...
        if len(self.int_schedule['TLState']) >= 0:
            ...
            for r, state in enumerate(le_state):
                vheader_item =  QTableWidgetItem('phase {}'.format(r))
    ...
    def get_data(self):
        ...
        tl_phase = self.int_schedule['TLState']
        for state_idx in range(synced_state):
            # 마지막 값은 duration
            if len(tl_phase) < state_idx + 1:
                tl_phase.append({'duration' : 0, 'lightcolor' : []})
            new_state_light = []
            for synced_ind in range(synced_header-1):
                new_state_light.append(self.tl_table.item(state_idx, synced_ind).text())
            tl_phase[state_idx]['lightcolor'] = new_state_light
            try:
                duration_time = int(self.tl_table.item(state_idx, synced_header-1).text())
            except:
                duration_time = 30
            tl_phase[state_idx]['duration'] = duration_time
        if len(self.int_schedule['yelloduration']) != len(self.int_item.TL):
            self.int_schedule['yelloduration'] = list([3]*len(self.int_item.TL))

        for i in range(self.tl_table.rowCount()):
            phase_name_list.append(self.tl_table.verticalHeaderItem(i).text())
        
        for phase_idx, phase in enumerate(tl_phase):
            phase_attrib = {'name':'','duration':''}

            # Each State
            traffic_signal_state_list = []
            for tl_group_index, tl_group in enumerate(self.int_item.TL):
                state_attrib = {'trafficSignalId':'', 'state':''}
                color = self.get_light_color(phase_idx, tl_group_index)
                state = None
                for tl in tl_group:
                    state_attrib['trafficSignalId'] = tl
                    state_attrib['state'] = color
                    state = TrafficSignalState(state_attrib)
                traffic_signal_state_list.append(state)
            # Phase Name
            phase_attrib['name'] = phase_name_list[phase_idx]
            
            # Add tl_state_list into Phase
            phase_attrib['duration'] = str(phase['duration'])
            phase_list.append(Phase(phase_attrib, traffic_signal_state_list, None))
        return phase_list
                    
    def get_light_color(self, phase_index, state_index):
        return self.int_schedule['TLState'][phase_index]['lightcolor'][state_index]
    ...
    @pyqtSlot()
    def __delete_phase(self):
        row = self.tl_table.rowCount()
        if len(self.int_schedule['TLState']) > 0:
            self.int_schedule['TLState'].pop()
            self.tl_table.removeRow(row)
            self.traffic_light_update_table_widget()
        else:
            Logger.log_warning("There is no phase to delete")

    def update_widget(self, id):
        self.id = id
        self.int_item = self.mgeo.intersection_controller_set.intersection_controllers[self.id]
        self.draw_gl.draw_item = self.int_item
        self.draw_gl.select_tl_list = None
        self.draw_gl._id = None
        try:
            self.draw_gl.center_point = calculate_centroid(self.int_item.get_intersection_controller_points())
            rectangle_box = minimum_bounding_rectangle(self.int_item.get_intersection_controller_points())
            self.draw_gl.zoom_factor = self.calculate_zoom_factor(rectangle_box)
        except:
            self.draw_gl.zoom_factor = -50

        self.int_schedule = {'idx' : self.id, 'TLState' : [], 'yelloduration' : [], 'PSState' : []}
        if id in self.mgeo.intersection_state_list:
            self.int_schedule['TLState'] = self.mgeo.intersection_state_list[self.id]['TLState'][:]
            self.int_schedule['yelloduration'] = self.mgeo.intersection_state_list[self.id]['yelloduration'][:]
            self.int_schedule['PSState'] = self.mgeo.intersection_state_list[self.id]['PSState'][:]
            
        self.traffic_light_update_table_widget()

## View Intersection Region Widget
class ViewIntersectionSchedule(QDialog):
    ...
    def initUI(self):
        ...
        main_layout.addWidget(split_data)
        self.setLayout(main_layout)
        self.setWindowTitle("View Intersection Phase")
    ...
    def set_tl_table_widget(self):
        main_widget = QGroupBox('')
        main_widget.setMinimumWidth(460)
        self.tl_table = QTableWidget()
        self.tl_table.setSelectionMode(QAbstractItemView.SingleSelection)
