...
def create_node_set_and_link_set(a1_node, a2_link, origin):
    '''
    Kaist의 GeoJSON으로부터 node_set과 link_set을 생성하여 리턴한다.
    '''

    """ GeoJSON의 A1_NODE로부터 NodeSet 타입 생성하기 """
    node_set = NodeSet()
    for i, item in enumerate(a1_node):
    ...
    set_lane_change_info_into_link_set(a2_link, link_set)

    return node_set, link_set


def set_lane_change_info_into_link_set(a2_link, link_set):
    '''
