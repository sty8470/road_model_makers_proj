...
def util_copy_link_lane_marking():
    Logger.log_info('util_copy_link_lane_marking called.')
    src_path = '../../saved/KATRI_210518_Change_Origin_V2'
    dst_path = '../../saved/KATRI_210518_SIM'
    ...
    for idx, src in src_mgeo.link_set.lines.items():
        if idx in dst_mgeo.link_set.lines.keys():
            dst = dst_mgeo.link_set.lines[idx]
            dst.lane_mark_left = src.lane_mark_left
            dst.lane_mark_right = src.lane_mark_right
        else:
            Logger.log_info('Maybe a newly created link: (not in dst_mgeo) id: {}'.format(idx))
    # dst_mgeo 파일에 저장
    dst_mgeo.to_json(dst_path)

    Logger.log_info('util_copy_link_lane_marking done OK.')


if __name__ == '__main__':
    util_copy_link_lane_marking()
