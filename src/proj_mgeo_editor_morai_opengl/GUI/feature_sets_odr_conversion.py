...
class OdrConversion(BaseFeatureSet):
    ...
    def clear_odr_roads(self):
        ...
        try:
            self.canvas.odr_data = OdrData(mgeo_planner_map=None)
    ...
    def create_prelimiary_odr_roads(self):
        ...
        """
        Logger.log_trace('Called: create_odr_road_preliminary')
        try:
            mgeo_key = self.canvas.mgeo_key
            mgeo_maps_dict = self.canvas.mgeo_maps_dict
            
            converter = MGeoToOdrDataConverter.get_instance()
            # [210507] road_id : int -> str
            self.canvas.odr_data = converter.create_preliminary_odr_roads(mgeo_maps_dict[mgeo_key], None)
            self.canvas.updateMgeoIdWidget()

            Logger.log_info('create_preliminary_odr_roads is done successfully.')
            return self.canvas.odr_data

        except BaseException as e:
            Logger.log_error('create_preliminary_odr_roads failed (traceback is down below) \n{}'.format(traceback.format_exc()))
    ...
    def create_odr_roads(self):
        ...
        try:
            ...
            converter = MGeoToOdrDataConverter.get_instance()
            self.canvas.odr_data = converter.create_odr_roads(mgeo_maps_dict[mgeo_key])
            ...
            try:
                ...
                if list_sp == []:
                    if self.odr_persistent_data != self.canvas.odr_data:
                        self.odr_persistent_data = None        
                    self.canvas.odr_data = converter.convert(mgeo_maps_dict[mgeo_key], self.odr_persistent_data)
                else:
                    if len(list_sp) > 1 and list_sp[0]['type'].name == 'ROAD':
                        Logger.log_trace('create_complete_odr_data is working with selected roads')
                        self.canvas.odr_data = converter.convert_selected(mgeo_maps_dict[mgeo_key], list_sp)
                    else:
                        if self.odr_persistent_data != self.canvas.odr_data:
                            self.odr_persistent_data = None
                        self.canvas.odr_data = converter.convert(mgeo_maps_dict[mgeo_key], self.odr_persistent_data)

                # self.canvas.odr_data = converter.convert(mgeo_planner_map)
                self.canvas.updateMgeoIdWidget()
                self.odr_persistent_data = self.canvas.odr_data # NOTE(hjp): change later

                end = time.time()
                Logger.log_trace('Elapsed Time: {} sec'.format(end - start))
