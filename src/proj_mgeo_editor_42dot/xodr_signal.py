...
class OdrSignal(object):
    ...
    @staticmethod
    def MGeoSignalTypeToOdrSignalType(mgeo_signal, force_change_to_carla_supported_ones=True):
        """
        ...
        if force_change_to_carla_supported_ones:
            odr_signal_type = 1000001 if mgeo_signal.dynamic == True else 205
        else:
            odr_signal_type = mgeo_signal.type
        
        # 현재는 subtype을 변경할 일이 없다. 그대로 사용
        odr_signal_sub_type = mgeo_signal.sub_type

        return odr_signal_type, odr_signal_sub_type
        ...
        for section_idx, lane_section in enumerate(road.get_lane_sections()):
            ...
            for v_idx, vector in enumerate(lane_section.geometry):
                ...
                if road.is_line(lane_section.geometry[v_idx]):
                    model = 'line'
                    # Logger.log_debug('      line'.format(v_idx))
                    pass
                elif len(lane_section.geometry[v_idx]) ==4:
                    model = 'poly3'
                    a = lane_section.geometry[v_idx][0]
                    b = lane_section.geometry[v_idx][1]
                    c = lane_section.geometry[v_idx][2]
                    d = lane_section.geometry[v_idx][3]
                else:
                    model = 'paramPoly3'
                    aU = lane_section.geometry[v_idx][0]
                    bU = lane_section.geometry[v_idx][1]
                    cU = lane_section.geometry[v_idx][2]
                    dU = lane_section.geometry[v_idx][3]
                    aV = lane_section.geometry[v_idx][4]
                    bV = lane_section.geometry[v_idx][5]
                    cV = lane_section.geometry[v_idx][6]
                    dV = lane_section.geometry[v_idx][7]
                    # Logger.log_debug('      poly3 = {} {} {} {}'.format(a,b,c,d))
