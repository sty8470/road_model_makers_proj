"""
Mobiletech에서 제공한 데이터를 이용하여 도로를 생성해본다
"""
...
from lib.common.sampling_nd import _insert_to_every_point
...
def get_outer_liner():
    # 여기서 데이터를 받으면 된다.
    input_path = '../../rsc/map_data/mobiltech_sample/'
    output_path = '../../output/mobiltech_sample/'
    relative_loc = True
    ...
    # Get Map Information
    map_info = GetShpFileDict(input_path)
    ...
    """
    origin = get_first_shp_point(map_info['polyline_01'])
    ...
    for key in map_info:
        ...
        shp_obj = map_info[key]
        shapes = shp_obj.shapes()

        print('len(shapes): ', len(shapes))
        # NOTE: len(shapes)가 1이기 때문에 가능하다
        ...
        for i in range(len(shapes)):
            ...
            # Origin을 전달하면 상대좌표로 변경
            if origin != None:
                shp_rec.points -= origin[0:2]
                shp_rec.z -= origin[2]
                pass
            ...
            # x,y,z 가 column인 배열을 따로 만들자
            xyz = np.c_[shp_rec.points, shp_rec.z]    
            x = xyz[:,0]
            y = xyz[:,1]
            z = xyz[:,1]
            ...
            if key == 'polyline_11':
                style = 'k'
                outer_upper = xyz.tolist()
            else:
                style = 'r' 
                inner_upper[key] = xyz.tolist()
    ...
    return outer_upper, inner_upper


def try_boolean_operation(outer_upper, inner_upper):
        # """
    # 기존 데이터
    # """    
    # outer_upper, inner_upper = _get_user_input()  
   ...
   # [STEP5] 출력
    ...
    objExporter.SetFilePrefix('test_road_with_hole_auto')
    objExporter.SetRenderWindow(renderWindow)
    objExporter.Write()


def simple_export(outer_upper, inner_upper_dict):
    # [STEP1] Outer Line으로 도로에 해당하는 vtkPolyData 타입의 객체를 만든다.
    thickness = 1
    outer_vertices = _make_vertices(outer_upper, thickness, raise_upper=False) # 꼭지점의 리스트 생성
    outer_faces = _make_face(len(outer_upper), normal_outside=True) # 면을 정의하는 리스트 생성 
    ...
    file_io.write_stl_and_obj(polyData_outer_upper, 'sample_road_outer_area')
    _plot_vtkPolyData(polyData_outer_upper, 'yellow')

    # [STEP2] Inner Line으로 구멍이 뚫려야하는 공간 (예를 들면 건물 부지)
    # 에 vtkPolyData 타입의 객체를 만든다.
    # (이 때 thickness를 훨씬 크게 해야한다.)
    for key in inner_upper_dict:
        inner_upper = inner_upper_dict[key]
        ...
        file_io.write_stl_and_obj(polyData_inner_upper, 'sample_road_inner_area_' + key)


if __name__ == '__main__':
    
    outer_upper, inner_upper_dict = get_outer_liner()

    """ 원래 해보려고 하던 작업 (Outer - Inner로 구멍 뚫어 도로만 만들어내기) """
    # try_boolean_operation(outer_upper, inner_upper_dict['polyline_01'])

    """ shp 파일 각각을 obj로 출력하는 파트 """
    simple_export(outer_upper, inner_upper_dict)
