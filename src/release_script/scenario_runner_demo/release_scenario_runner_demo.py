...
"""빌드하려는 프로그램 경로"""
src_path = os.path.normpath(os.path.join(prj_root_path, 'src/proj_scenario_runner_demo'))
...
def build():
    ...
    spec_file_path = os.path.normpath(os.path.join(src_path, 'scenario_runner_demo.spec'))
    shutil.copy(os.path.join(current_path, 'scenario_runner_demo.spec'), spec_file_path)
    ...
    """ 파일 Copy """
    src = os.path.normpath(os.path.join(src_path, 'dist/scenario_runner_demo/'))
    print('[INFO] Copy Package files {} to {}'.format(src, release_path))
    shutil.copytree(src, os.path.normpath(os.path.join(release_path, 'program')))

    src = os.path.normpath(os.path.join(src_path, 'sample_openscenario'))
    print('[INFO] Copy Package files {} to {}'.format(src, release_path))
    shutil.copytree(src, os.path.normpath(os.path.join(release_path, 'sample_openscenario')))
    ...
    try:
        ...
        if platform.system() == "Windows":
            ...
            cmd = 'ln -s ./program/scenario_runner_demo'
