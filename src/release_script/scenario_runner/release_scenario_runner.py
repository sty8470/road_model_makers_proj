...
def build(release_version = None):     
    ...
    folder_name = 'scenario_runner_{}'.format(path_utils.get_now_str(include_sec=True))
    ...
    # 임시로 빌드되는 경로
    dst_user_path = os.path.abspath(os.path.join(package_proj_path, 'dist/main/proj_mgeo_editor_morai_opengl', 'User.json'))
    about_path = os.path.abspath(os.path.join(package_proj_path, 'dist/main/proj_mgeo_editor_morai_opengl', 'about.json'))
    mgeo_editor_path = os.path.abspath(os.path.join(current_path, '../../proj_mgeo_editor_morai_opengl'))
    src_user_path = os.path.abspath(os.path.join(mgeo_editor_path, 'GUI', 'Users', 'User_ScenarioRunner.json'))


    """ 패키징을 위해 spec 파일 복사 """
    spec_file_path = os.path.join(package_proj_path, 'main.spec')
    shutil.copy(os.path.join(current_path, 'main.spec'), spec_file_path)
    ...
    """ 파일 Copy """
    src = os.path.normpath(os.path.join(package_proj_path, 'dist/main/'))
    ...
    try:
        ...
        #     cmd = 'ln -s ./program/scenario runner'
